...
The Computer Science Hub AI platform is built using a modern, serverless-first architecture leveraging Next.js 14 App Router. It follows a monolithic repository (monorepo) approach for the frontend, backend, and database configurations.
Tech Stack
...
Data Flow
    ...
    Client Request: User interacts with the React UI.
    API Route: Next.js Route Handler receives the request, validates session via NextAuth.
    Database: Prisma Client queries the PostgreSQL database.
    AI Stream: If an AI request, the Vercel AI SDK streams chunks from OpenAI back to the client via Server-Sent Events (SSE).
    Response: JSON data or a streaming text response is returned to the client.
